...
public class Deque<Item> implements Iterable<Item> {
    ...
    public void addFirst(Item item) {
        if (item == null) {
            throw new NullPointerException(
                    "Attempted to add a null pointer to the beginning of deque");
        }
    ...
    public void addLast(Item item) {
        if (item == null) {
            throw new NullPointerException(
                    "Attempted to add a null pointer to the end of deque");
        }
    ...
    public Item removeFirst() {
        if (first == null) {
            throw new NoSuchElementException(
                    "Attempted to remove an item from an empty deque");
        }
        Node oldFirst = first;
        first = oldFirst.next;
    ...
    private class DequeIterator implements Iterator<Item> {
        ...
        public Item next() {
            if (!hasNext())
                throw new NoSuchElementException(
                        "There are no more items on the deque");
    ...
    public static void main(String[] args) {
        ...
        if (!(testD.size() == 0)) {
            ...
            StdOut.println("Pass: size() == 0 after constructor");
        testD.addFirst("firstString");
        testD.addFirst("secondString");
        testD.addFirst("thirdString");
        testD.addLast("fourthString");
        testD.addLast("fithString");
        testD.addLast("sixthString");
        testD.addLast("sevethString");
        ...
        for (int k = 0; k < size; k++) {
            ...
            StdOut.println("testD.removeFirst = " + testD.removeFirst()
                    + " testD.size() = " + testD.size());
        }

        StdOut.println("Removed all size = " + testD.size());
        testD.addFirst("newFirstString");
        if (testD.size() != 1)
            StdOut.println("Error: size did not increase to = 1");
        else
            StdOut.println("Pass: added newFirstString: size = 1 ");
        size = testD.size();
        for (int k = 0; k < size; k++) {
            // StdOut.println("size = " + testD.size());
            StdOut.println("testD.removeFirst() = " + testD.removeFirst()
        ...
        testD.addLast("newLastString");
        if (testD.size() != 1)
            StdOut.println("Error: size did not increase to = 1");
        else
            StdOut.println("Pass: Size increased size = 1 ");
        size = testD.size();
        for (int k = 0; k < size; k++) {
            StdOut.println("size = " + testD.size());
        ...
        if (testD.size() != 3)
            ...
            StdOut.println("Pass: Size increased size = 3 ");
        size = testD.size();
        for (int k = 0; k < size; k++) {
            StdOut.println("size = " + testD.size());
